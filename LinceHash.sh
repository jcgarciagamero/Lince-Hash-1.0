...
file=$(zenity --width=360 --height=320 --list --title "Lince Hash 1.0" --column Descripción "Imagen de disco Y Hash" "Calcular hash de imagen" "Comparar Hash")
var1=$(echo $?)
...
if [ $var1 = 1 ]; then
	...
	sudo "$directorio/LinceIS.sh"

elif [ "$file" = "Calcular hash de imagen" ]; then
	sudo "$directorio/Linceh.sh"

elif [ "$file" = "Comparar Hash" ]; then
	sudo "$directorio/LinceComp.sh"
